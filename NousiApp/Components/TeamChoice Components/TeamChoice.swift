        ...
        ForEach (choices, id: \.id) { choice in
            VStack (alignment: .leading) {
                HStack {
                    ...
                    Text("\(com_NL_TeamsChoice.isChoicePositiveIndicator(input: choice.priceChangePercentage24h))\(String(format: "%.2f", choice.priceChangePercentage24h))%")
                        .font(.system(size: 14))
                        .fontWeight(.bold)
                        .foregroundColor(com_NL_TeamsChoice.isChoicePositiveColor(input: choice.priceChangePercentage24h))
                }
